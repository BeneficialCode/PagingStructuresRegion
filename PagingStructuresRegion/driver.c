...
ULONG_PTR g_PTE_BASE = 0xFFFF818000000000;
ULONG_PTR g_PDE_BASE = 0xFFFF81C0C0000000;
ULONG_PTR g_PPE_BASE = 0xFFFF81C0E0600000;
ULONG_PTR g_PXE_BASE = 0xFFFF81C0E0703000;
...
void PrintEntryInfo(PMMPTE pEntry) {
	KdPrint(("Copy on write:%d,Global:%d,Large page:%d,\
            Dirty:%d, Accessed: %d,Cache disabled: %d,Write-through: %d,Owner: %d,Writeable or read-only: %d,\
            Valid: %d,Executable page: %d\n", pEntry->u.Hard.CopyOnWrite, pEntry->u.Hard.Global,pEntry->u.Hard.LargePage, pEntry->u.Hard.Dirty,
		pEntry->u.Hard.Accessed, pEntry->u.Hard.CacheDisable, pEntry->u.Hard.WriteThrough, pEntry->u.Hard.Owner,
		pEntry->u.Hard.Dirty1, pEntry->u.Hard.Valid, pEntry->u.Hard.NoExecute));
}
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {
	...
	void* pNtCreateFile = (void*)NtCreateFile;
	PMMPTE pPDE = GetPDEAddress(pNtCreateFile);
	KdPrint(("PDE: %p\n", pPDE));
	if (pPDE->u.Hard.LargePage) {
		PrintEntryInfo(pPDE);
	}
	else {
		PMMPTE pPTE = GetPTEAddress(pNtCreateFile);
		KdPrint(("PTE: %p\n", pPTE));
		PrintEntryInfo(pPTE);
	}

	void* addr = (void*)0xfffff807'84400000;
	pPDE = GetPDEAddress(addr);
	KdPrint(("PDE: %p\n", pPDE));
	if (pPDE->u.Hard.LargePage) {
		PrintEntryInfo(pPDE);
	}
	else {
		PMMPTE pPTE = GetPTEAddress(addr);
		KdPrint(("PTE: %p\n", pPTE));
		PrintEntryInfo(pPTE);
	}
